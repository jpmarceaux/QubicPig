...
class JobManager:
    ...
    def __init__(self, chipspec, fpga_config, channel_configs):
        ...
        self.readout_chanmap = {qid: channel_configs[qid + '.rdlo'].core_ind for qid in chipspec.spec.qubits}
    ...
    def collect_pygsti_dataset(self, pcircs_and_qinstructions, num_shots, circuit_runner, new_chipspec=None):
        ...
        for circ_idx, circ in enumerate(pcircs_and_qinstructions.keys()):
            ...
            for shot_idx in range(num_shots):
                shot_string = ''
                for qid in self.chipspec.spec.qubits:
                    shot_string = shot_string.join(str(s) for s in raw_iq_shots[circ_idx][qid][shot_idx])
